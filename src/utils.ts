import PhaserRND from "#app/phaser/phaserRND";
...
export function randSeedGauss(stdev: number, mean: number = 0): number {
  ...
  const u = 1 - PhaserRND.realInRange(0, 1);
  const v = PhaserRND.realInRange(0, 1);
  const z = Math.sqrt(-2.0 * Math.log(u)) * Math.cos(2.0 * Math.PI * v);
  return z * stdev + mean;
}
...
export function randSeedInt(range: integer, min: integer = 0): integer {
  ...
  return PhaserRND.integerInRange(min, (range - 1) + min);
}
...
export function randSeedItem<T>(items: T[]): T {
  return items.length === 1
    ? items[0]
    : PhaserRND.pick(items);
}

export function randSeedWeightedItem<T>(items: T[]): T {
  return items.length === 1
    ? items[0]
    : PhaserRND.weightedPick(items);
}
...
export function randSeedEasedWeightedItem<T>(items: T[], easingFunction: string = "Sine.easeIn"): T {
  ...
  const value = PhaserRND.realInRange(0, 1);
  const easedValue = PhaserTweens.GetEaseFunction(easingFunction)(value);
...
export function randSeedShuffle<T>(items: T[]): T[] {
  ...
  for (let i = items.length - 1; i > 0; i--) {
    const j = PhaserRND.integerInRange(0, i);
 ...
 */
export function printContainerList(container: PhaserContainer): void {

...
export function addWindow(scene: BattleScene, x: number, y: number, width: number, height: number, mergeMaskTop?: boolean, mergeMaskLeft?: boolean, maskOffsetX?: number, maskOffsetY?: number, windowVariant?: WindowVariant): PhaserNineSlice {
...
export function updateWindowType(scene: BattleScene, windowTypeIndex: integer): void {
  const windowObjects: [PhaserNineSlice, WindowVariant][] = [];
  const themedObjects: (PhaserImage | PhaserNineSlice | PhaserSprite)[] = [];
  const traverse = (object: any) => {
    if (object.hasOwnProperty("children") && object.children instanceof PhaserDisplayList) {
      const children = object.children as PhaserDisplayList;
      for (const child of children.getAll()) {
        traverse(child);
      }
    } else if (object instanceof PhaserContainer) {
      for (const child of object.getAll()) {
        traverse(child);
      }
    } else if (object instanceof PhaserNineSlice) {
    ...
    } else if (object instanceof PhaserSprite) {
...
export function addUiThemeOverrides(scene: BattleScene): void {
  ...
  scene.add.image = function (x: number, y: number, texture: string | PhaserTexture, frame?: string | number): PhaserImage {
    let legacy = false;
    if (typeof texture === "string" && scene.uiTheme && legacyCompatibleImages.includes(texture)) {
      legacy = true;
      texture += "_legacy";
    }
    const ret: PhaserImage = originalAddImage.apply(this, [ x, y, texture, frame ]);
    if (legacy) {
  ...
  scene.add.sprite = function (x: number, y: number, texture: string | PhaserTexture, frame?: string | number): PhaserSprite {
    let legacy = false;
    if (typeof texture === "string" && scene.uiTheme && legacyCompatibleImages.includes(texture)) {
      legacy = true;
      texture += "_legacy";
    }
    const ret: PhaserSprite = originalAddSprite.apply(this, [ x, y, texture, frame ]);
    if (legacy) {
      const originalSetTexture = ret.setTexture;
      ret.setTexture = function (key: string, frame?: string | number) {
        key += "_legacy";
  ...
  scene.add.nineslice = function (x: number, y: number, texture: string | PhaserTexture, frame?: string | number, width?: number, height?: number, leftWidth?: number, rightWidth?: number, topHeight?: number, bottomHeight?: number): PhaserNineSlice {
    let legacy = false;
    if (typeof texture === "string" && scene.uiTheme && legacyCompatibleImages.includes(texture)) {
      legacy = true;
      texture += "_legacy";
    }
    const ret: PhaserNineSlice = originalAddNineslice.apply(this, [ x, y, texture, frame, width, height, leftWidth, rightWidth, topHeight, bottomHeight ]);
    if (legacy) {
      const originalSetTexture = ret.setTexture;
      ret.setTexture = function (key: string | PhaserTexture, frame?: string | number, updateSize?: boolean, updateOrigin?: boolean) {
        key += "_legacy";
